(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var store = $rdf.graph();
    var fetcher = new $rdf.Fetcher(store, undefined);
    ...
    function statementMatches(statement, subjectRef, predicateRef, objectRef, documentRef) {
        ...
        var targetDocument = $rdf.sym(documentRef);
    ...
    function toNode(referenceOrBlankNode) {
        return (typeof referenceOrBlankNode === 'string') ? $rdf.sym(referenceOrBlankNode) : referenceOrBlankNode;
    }
    ...
    function initialiseSubject(document, subjectRef) {
        var subjectNode = isBlankNode$1(subjectRef) ? subjectRef : $rdf.sym(subjectRef);
        ...
        var addLiteral = function (predicateRef, literal) {
            pendingAdditions.push($rdf.st(subjectNode, $rdf.sym(predicateRef), asLiteral(literal), $rdf.sym(document.asRef())));
        };
        var addRef = function (predicateRef, ref) {
            pendingAdditions.push($rdf.st(subjectNode, $rdf.sym(predicateRef), $rdf.sym(ref), $rdf.sym(document.asRef())));
        };
        var removeRef = function (predicateRef, nodeRef) {
            pendingDeletions.push($rdf.st(subjectNode, $rdf.sym(predicateRef), $rdf.sym(nodeRef), $rdf.sym(document.asRef())));
        };
        ...
        var subject = {
            ...
            removeLiteral: function (predicateRef, literal) {
                pendingDeletions.push($rdf.st(subjectNode, $rdf.sym(predicateRef), asLiteral(literal), $rdf.sym(document.asRef())));
            },
            removeRef: removeRef,
    ...
    function fromDateTimeLiteral(literal) {
        // See https://github.com/linkeddata/$rdf.js/blob/d84af88f367b8b5f617c753d8241c5a2035458e8/src/literal.js#L87
    ...
    function asLiteral(literal) {
        if (literal instanceof Date) {
            return $rdf.Literal.fromDate(literal);
        }
        if (typeof literal === 'number') {
            return $rdf.Literal.fromNumber(literal);
        }
        return new $rdf.Literal(literal, undefined, undefined);
    }
    ...
    function instantiateDocument(uri, metadata) {
        ...
        var statements = getStore().statementsMatching(null, null, null, $rdf.sym(documentRef));
